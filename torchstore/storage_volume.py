...
class InMemoryStore(StorageImpl):
    ...
    def _extract_sharded_tensor(self, request: Request, shards) -> Optional[torch.Tensor]:
        for shard in shards:
    ...
    async def get(
        ...
        extracted_tensor = self._extract_sharded_tensor(request, self.kv[key].values())
    ...
    async def get_meta(
        ...
        if request is not None and request.tensor_slice is not None:
            extracted_tensor = self._extract_sharded_tensor(request, stored_object.values())
            if extracted_tensor is not None:
        ...
        raise RuntimeError(f"Unknown type for {key} type={type(stored_object)} {stored_object=}")
