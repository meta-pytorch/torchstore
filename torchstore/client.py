...
class LocalClient:
    ...
    async def _fetch(
        ...
        fetch_pairs = await self._fetch_results(
            volume_requests, transport_buffer_map
        )
    ...
    async def _fetch_results(
        ...
        per_volume = await asyncio.gather(
            *[
                _fetch_one(vid, reqs)
                for vid, reqs in volume_requests.items()
            ]
        )
    ...
    def _assemble_results(
        ...
        for sub_request, result in fetch_pairs:
            if sub_request.key in direct_keys:
                ...
                slice_parts[sub_request.key].append(
                    (result, sub_request.tensor_slice)
                )
