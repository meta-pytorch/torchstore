...
try:
    from torchcomms._transport import RdmaMemory,  RdmaTransport, RdmaRemoteBuffer
...
class TransportContext:
    ...
    def __init__(self):
        ...
        self.transport_context[self.RDMA_TRANSPORT_CACHE] = RdmaTransportCache.try_init()
...
class TransportBuffer:
    ...
    async def read_into(self, tensor: Optional[torch.Tensor], transport_context: TransportContext) -> torch.Tensor:
        raise NotImplementedError()

    async def write_from(self, tensor: Optional[torch.Tensor], transport_context: TransportContext) -> None:
        raise NotImplementedError()

    async def handshake(self, tensor: torch.Tensor, volume_ref: "StorageVolumeRef") -> None:
        """Establish a handshake with the remote volume, such as for RDMA."""
        pass

    async def recv_handshake(self, transport_context: TransportContext) -> Optional[Any]:
    ...
    def _assert_valid_tensor(
        self, tensor: torch.Tensor, dtype: torch.dtype, shape: torch.Size, must_be_contiguous = False
    ) -> None:
...
class TorchCommsRdmaTransportBuffer(TransportBuffer):
    ...
    def __init__(self) -> None:
        ...
        self.tensor_ref: torch.Tensor | None = None # reference to local client's destination tensor
        self.rdma_memory: RdmaMemory | None = None # must be kept alive until transport is done
        self.rdma_remote_buffer: RdmaRemoteBuffer | None = None # remote reference of rdma memory
    ...
    async def handshake(self, tensor: torch.Tensor, volume_ref: "StorageVolumeRef") -> None:
        ...
        local_transport, self.address = transport_cache.get(volume_ref.volume_id, device)
    ...
    async def recv_handshake(self, transport_context: TransportContext) -> Optional[Any]:
        ...
        """
        transport, addr = transport_context.get_rdma_transport_cache().put(self.address, device=0)
    ...
    async def read_into(self, tensor: Optional[torch.Tensor], transport_context: TransportContext) -> torch.Tensor:
        ...
        transport = transport_context.get_rdma_transport_cache().get(self.address, 0)[0]

        receiving_buffer = RdmaMemory(tensor)
        res = transport.read(receiving_buffer.to_mutable_view(), self.rdma_remote_buffer)
    ...
    async def write_from(self, tensor: Optional[torch.Tensor], transport_context: TransportContext) -> None:
        ...
        if not tensor.is_contiguous():
            contiguous_buffer = torch.empty(tensor.shape, dtype=tensor.dtype, device='cpu',
                  memory_format=torch.contiguous_format, pin_memory=True)
...
class RDMATransportBuffer(TransportBuffer):
    ...
    async def write_from(self, tensor: Optional[torch.Tensor], transport_context: TransportContext) -> None:
        ...
        self._assert_valid_tensor(tensor, self.dtype, self.shape, must_be_contiguous=False)
        assert self.rdma_buffers is not None
