...
class RDMATransportBuffer(TransportBuffer):
    ...
    def _assert_valid_tensor(self, tensor: torch.Tensor, must_be_contiguous: bool=True) -> None:
    ...
    async def write_from(self, tensor: Optional[torch.Tensor]) -> None:
        if tensor is None:
            return
        # soure tensor does not have to be contiguous, it is copied into contiguous memory later in this function
