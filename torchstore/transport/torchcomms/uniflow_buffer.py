...
class TorchCommsTransportBuffer(TransportBuffer):
    ...
    def __init__(self, storage_volume_ref: StorageVolumeRef) -> None:
        ...
        self.peer_key = storage_volume_ref.transport_context.get(UniflowCache).peer_key(
    ...
    def _get_sv_transport(
        ...
        if device_id in self._connections:
            ...
            return uniflow_cache.get_handshake_transport(self.handshake_id, device_id)
    ...
    async def recv_handshake(
        ...
        raise RuntimeError(
            f"Unknown TorchComms handshake phase: {self._handshake_phase}"
        )
    ...
    def _transfer_request(
        ...
        request = cast(
            "TransferRequest", module.TransferRequest(local_span, remote_span)
        )
