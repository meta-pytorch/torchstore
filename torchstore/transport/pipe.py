...
class Pipe:
    ...
    async def get_from_storage_volume(self, key, request: Request):
        ...
        try:
            await transport_buffer.handshake(request.tensor_val, self.storage_volume_ref)
            allocate = transport_buffer.allocate_dest if is_torchcomms_rdma else transport_buffer.allocate
            ...
            if not is_torchcomms_rdma:
                return await transport_buffer.read_into(request.tensor_val, self.transport_context)
            else:
