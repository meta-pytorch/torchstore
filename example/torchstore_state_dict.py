...
def _make_state_dict(seed: int) -> OrderedDict[str, torch.Tensor]:
    ...
    g = torch.Generator(device=ACCEL).manual_seed(seed)
    sd: OrderedDict[str, torch.Tensor] = OrderedDict()
    for i in range(4):
        sd[f"layer.{i}.lora_A.weight"] = torch.randn(8, 16, generator=g, device=ACCEL)
        sd[f"layer.{i}.lora_B.weight"] = torch.randn(16, 8, generator=g, device=ACCEL)
    return sd
