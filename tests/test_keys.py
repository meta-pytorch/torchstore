...
async def test_keys_basic():
    ...
    assert await ts.keys() == [".x", "v0.x", "v0.y", "v0.x.z", "v1.x", "v1.y"]
    assert await ts.keys("v0") == ["v0.x", "v0.y", "v0.x.z"]
    assert await ts.keys("v0.x") == ["v0.x", "v0.x.z"]
    assert await ts.keys("v0.x.z") == ["v0.x.z"]
    assert await ts.keys("") == ["", ".x"]
    assert await ts.keys("v1") == ["v1.x", "v1.y"]
...
async def test_keys_multi_process():
    ...
    for rank in range(volume_world_size):
        assert await ts.keys(f"key_{rank}") == [f"key_{rank}.t1", f"key_{rank}.t2"]
