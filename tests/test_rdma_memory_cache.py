...
"""Unit tests for RdmaMemoryCache with mocked RdmaMemory."""
...
# ---------------------------------------------------------------------------
# Mock RdmaMemory so tests don't require torchcomms
# ---------------------------------------------------------------------------
...
class MockRdmaMemory:
    ...
    Deliberately does NOT hold a reference to the tensor — matching the real
    torchcomms RdmaMemory behaviour (stores raw pointer, not tensor).
    """
...
# Patch RdmaMemory *before* importing the cache module so the guarded import
...
class TestRdmaMemoryCacheWeakrefEviction:
    def test_evicts_when_tensor_is_deleted(self):
        ...
        del t
        # CPython refcount-based GC fires the weakref callback synchronously
